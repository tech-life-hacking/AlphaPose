        ...
        for i, (start_p, end_p) in enumerate(l_pair):
            if start_p in part_line and end_p in part_line:
                ...
                cv2.line(img, start_xy, end_xy,
                         line_color[i], 2*(kp_scores[start_p] + kp_scores[end_p]) + 1)
    return img
...
def vis_frame(frame, im_res, format='coco'):
    ...
    for human in im_res['result']:
        ...
        for n in range(kp_scores.shape[0]):
            ...
            img = cv2.addWeighted(bg, float(transparency),
                                  img, float(1-transparency), 0)
        # Draw limbs
        for i, (start_p, end_p) in enumerate(l_pair):
            if start_p in part_line and end_p in part_line:
                start_xy = part_line[start_p]
                ...
                img = cv2.addWeighted(
                    bg, float(transparency), img, float(1-transparency), 0)
